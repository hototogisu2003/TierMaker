...
import { coerceBingoPayload, insertBingoSubmission } from "@/lib/bingo/server";

export const dynamic = "force-dynamic";

const BINGO_DEVICE_COOKIE_NAME = "bingo_device_token";
...
function setDeviceTokenCookie(response: NextResponse, deviceToken: string) {
  response.cookies.set(BINGO_DEVICE_COOKIE_NAME, deviceToken, {
    httpOnly: true,
    sameSite: "lax",
...
export async function POST(request: Request) {
  ...
  let deviceToken = normalizeDeviceToken(cookieStore.get(BINGO_DEVICE_COOKIE_NAME)?.value);
  ...
  try {
    ...
    const result = await insertBingoSubmission(payload, deviceToken);
